global port_names: table[port] of string = {
	[22/tcp]="ssh",
	[80/tcp]="http",
	[433/tcp]="http_443",
	[8001/tcp]="http_8001", # not sure
	[2784/tcp]="http_2784", # not sure
	};
global num_conn=0;
...
function record_connectionTCP(c: connection){
	...
	# konexioaktcp[startTime, orig_h, orig_p, resp_h, resp_p]$service = service_name(resp_p); # append
}
...
event udp_contents(u: connection, is_orig: bool, contents: string)
{
	...
	# konexioak[startTime, duration, orig_h, orig_p, resp_h, resp_p]$service = service_name(resp_p); # append
...
event bro_done()
{
	...
	for ([startTimet, orig_ht, orig_pt, resp_ht, resp_pt] in konexioaktcp){
		...
		# ,konexioaktcp[startTimet, orig_ht, orig_pt, resp_ht, resp_pt]$service); #append
  ...
  for ([startTime, duration, orig_h, orig_p, resp_h, resp_p] in konexioak)
  {
    ...
    if(konexioak[startTime, duration, orig_h, orig_p, resp_h, resp_p]$protokoloa == "icmp"){
       	...
       	konexioak[startTime, duration, orig_h, orig_p, resp_h, resp_p]$orig_p,
			...
			# ,konexioak[startTime, duration, orig_h, orig_p, resp_h, resp_p]$service); #append
		}
		#udp
